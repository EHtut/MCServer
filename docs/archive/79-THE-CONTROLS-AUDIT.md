> ⛔ **ARCHIVED 2026-09-05 — FICTION UNLESS RE-VERIFIED.** Its subject is spent: The pass shipped (V went from 11 actions to 1) and the doc's own CORRECTIONS section records that the audit which proposed it was wrong twice.
>
> **HOLDS (why this was archived and not deleted):** The two corrections and the recorded false alarm so neither is repeated; the durable rule that core controls are cleared and nothing else may sit on them ('jumping should not deploy a siege ladder'); the packwiz delivery problem for pushing options.txt to players; and the two items left genuinely open.
>
> ⚠️ **KNOWN STALE:** Its headline '76% of every bound action is fighting another one' is retracted further down as counting co-location rather than conflict, and the claim that the 93 unbound are unreachable path content is retracted outright — both wrong claims still stand above their corrections.
>

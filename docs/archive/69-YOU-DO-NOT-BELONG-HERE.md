> ⛔ **ARCHIVED 2026-09-05 — FICTION UNLESS RE-VERIFIED.** Its subject is spent: F1 shipped as trespass.js the same day, and the two addenda close the depth question rather than opening work.
>
> **HOLDS (why this was archived and not deleted):** Ethan's EIGHT LINES VERBATIM (four Nether, four End) — his writing, and the highest-value thing in the doc; the standing rule that these must never be attributed to a god or the Nether becomes another place a god can reach you; and the addendum that REFUTED 64's depth diagnosis with a measured per-slice profile.
>
> ⚠️ **KNOWN STALE:** Its own header notes it said DRAFT/'zero code' after the code shipped; the §91 'OPEN — needed before this is built' list is answered by the addenda below it.
>

> ⛔ **ARCHIVED 2026-09-05 — FICTION UNLESS RE-VERIFIED.** Its subject is spent: Superseded twice — 69's addendum proved the diagnosis wrong, and min_y was ruled back to −64 and shipped, with pack/datapacks/mcserver_depth deleted outright.
>
> **HOLDS (why this was archived and not deleted):** The rcon method note (`execute if block` returns a real result; the `run say` form matches its own echo and reports every block as air); the lesson that worldgen is baked into level.dat so a config change cannot fix an existing world even for ungenerated chunks; the tectonic.json.bak-2026-08-24 pointer.
>
> ⚠️ **KNOWN STALE:** 'DIAGNOSED, FIXED, AND VERIFIED' and the staged min_y −128 fix; both were reversed, and the doc still tells a future reader to check the fix at regen time.
>

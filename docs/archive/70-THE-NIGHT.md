> ⛔ **ARCHIVED 2026-09-05 — FICTION UNLESS RE-VERIFIED.** Its subject is spent: D1–D6 all shipped and are live (night.js, tide.js, waves.js); what is left is one open question, not a build plan.
>
> **HOLDS (why this was archived and not deleted):** Ethan's constraint 'There is a lore reason that they don't know about. You don't know either' with the explicit DO NOT INVENT ONE; the per-god silenced/speaks table; the deliberate no-inversion cost (Wall and Forge start at max trust so their champions get the hardest nights immediately); and the OPEN problem that the night danger cannot reach a player indoors, which gates D3's value, appears in no other doc, and is not in DEFECTS.md.
>
> ⚠️ **KNOWN STALE:** Its own header notes it said DRAFT/'zero code' after six chunks shipped; the Iron's-suite and miniboss-collision sections are resolved in place.
>

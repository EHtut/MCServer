> ⛔ **ARCHIVED 2026-09-05 — FICTION UNLESS RE-VERIFIED.** Its subject is spent: It is the build plan for a system that is retired, and its own STATE OF THE BUILD table admits it was frozen at 11:11 on 2026-08-05 with fourteen commits landing after it.
>
> **HOLDS (why this was archived and not deleted):** The C0 capability-probe results — hard-won KubeJS facts that are still true tree-wide (`EntityEvents.hurt` does not exist, `event.cancel()` unwinds by throwing, `removeModifier` is unusable, `global` cannot be assigned) — plus the measured numbers (drop rate 13.4% expected vs 13.3% observed, live armour 9.08, chaff health 16.35/15.0/5.3/21.8) and the `persistentData`-survives-death-via-`PlayerEvent.Clone` finding.
>
> ⚠️ **KNOWN STALE:** Every ✅ in its verification table is stamped "true as of 11:11" by its own admission, and the "what to test when everyone is back" list is four-player work that will never happen.
>

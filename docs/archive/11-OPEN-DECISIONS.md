> ⛔ **ARCHIVED 2026-09-05 — FICTION UNLESS RE-VERIFIED.** Its subject is spent: Self-described as "most of this was answered", it is a history doc, and history belongs in git and in the canon docs that absorbed each ruling; its live-looking entries are now the stalest thing in it.
>
> **HOLDS (why this was archived and not deleted):** Ethan verbatim, still canon: "the entities are no longer the actual patrons, they are instead the actors of them" — gods speak, actors arrive, and a bug in a servant is smaller than a bug in a god. Also the world-reset capture ("halfway kinda wanting to reset... but lets finish and get all this working first") with the observation that a reset does I4's job for free and shrinks it to an admin tool; the measured finding that the pack ships a skill engine (`puffish_skills`, 0 data files) plus 254 datapack JSONs of content for it and configures neither; the A1–A6 depth-loop rulings; and the D "settled — do not re-open" table.
>
> ⚠️ **KNOWN STALE:** Its chat-only resolution is cited as settled by 23 and 34, and the gods have since moved off the chat bar to Immersive Messages overlays with `voice.js` CHAT_COPY false; the live roster it names (Rehykt=forge, Lehykt=blade, j0nesyboi223=salvage) is dead, and several decisions are moot under one player.
>

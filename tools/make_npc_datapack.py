...
def emit_nbt_js(presets, repo):
    """Write the four tiers' entity NBT as a KubeJS constant.

    🔴 WHY THIS EXISTS. `/easy_npc preset import_new` is the mod's own way to create an NPC
    from a preset, and it does not work here: every source (custom/data/default/world) answers
    "Unable to import ... preset", INCLUDING for the mod's own shipped `easy_npc:base/allay`.
    So it is not our resource path - the route itself is unavailable to us.

    ⭐ WHAT DOES WORK, PROVED IN THE WORLD 2026-09-06:

        summon easy_npc:humanoid <x> <y> <z> <the preset's `data` block>

    and the entity that appears has Invulnerable:1b, the wheat Offers, and the STANDARD
    DialogData - checked field by field with `data get entity`.

    ⚠️ THE PRESET FILES STAY. They are the source this is generated from, they are how the
    NBT is authored and reviewed, and `import_new` may work in a later version of the mod.
    This is a second consumer of the same data, not a replacement for it.

    ⛔ AND IT CANNOT BE TESTED OVER RCON. The NBT is ~3.5 KB and rcon's packet limit is about
    1.4 KB, so the command is silently truncated and the summon fails with no error. That is
    how this looked broken for a while. A datapack function has no such limit and is how it
    was finally proved.
    """
    import io as _io
    out = []
    out.append("// ank_nbt.js - GENERATED by tools/make_npc_datapack.py. Do not hand-edit.")
    out.append("//")
    out.append("// The entity NBT for each of Ank's four price tiers, lifted from the `data`")
    out.append("// block of the matching preset. ank.js summons with these because")
    out.append("// `/easy_npc preset import_new` does not work on this build - see the")
    out.append("// generator's emit_nbt_js docstring for what was tried and what proved out.")
    out.append("var VELDORA = (typeof VELDORA !== 'undefined') ? VELDORA : {};")
    out.append("")
    out.append(";(function () {")
    out.append("  VELDORA.ankNbt = {")
    for name, path in presets:
        ...
        while i < len(t):
            ...
            if c == '{':
                ...
                depth -= 1
            elif depth == 1 and t.startswith('data:', i) and (i == 0 or not t[i - 1].isalnum()):
                start = i + len('data:')
                break
            i += 1
        if start is None:
            raise SystemExit('no top-level data: block in ' + path)
        j = t.index('{', start)
        d = 0
        for k in range(j, len(t)):
            if t[k] == '{':
                ...
                if d == 0:
                    ...
                    break
        nbt = ' '.join(t[j:end].split())
        # ⚠️ HIS WRITING IS FULL OF APOSTROPHES - "I don't recommend that", "Watcha
        # buyin'" - so the JS string wrapper has to escape them. The first version of this
        # emitter refused rather than escaping, on the assumption there were none; the
        # guard fired on the first run, which is the right way round for a guard to be
        # wrong. Backslash first, or it doubles the escape it just added.
        esc = nbt.replace(chr(92), chr(92) * 2).replace("'", chr(92) + "'")
        out.append("    '%s': '%s'," % (name, esc))
    out.append("  }")
    out.append("")
    out.append("  ServerEvents.loaded(function () {")
    out.append("    var n = 0")
    out.append("    for (var k in VELDORA.ankNbt) if (VELDORA.ankNbt.hasOwnProperty(k)) n++")
    out.append("    console.info('[ank] ' + n + ' preset NBT block(s) loaded for summoning.')")
    out.append("  })")
    out.append("})();")
    dest = os.path.join(repo, 'pack', 'kubejs', 'server_scripts', 'ank_nbt.js')
    NL = chr(10)
    blob = (NL.join(out) + NL).encode('utf-8')
    open(dest + '.tmp', 'wb').write(blob)
    os.replace(dest + '.tmp', dest)
    print("  wrote pack/kubejs/server_scripts/ank_nbt.js (%d tier(s))" % len(presets))
...
def main():
    ...
    # ⭐ AND THE SAME DATA AS A KUBEJS CONSTANT, because the mod's own import route does
    # not work on this build - see emit_nbt_js. Generated from the FIRST preset dir so the
    # NBT and the files can never disagree.
    tiers_out = []
    d0 = PRESET_DIRS[0]
    for f in sorted(os.listdir(d0)):
        if f.startswith("ank_t") and f.endswith(".npc.snbt"):
            tiers_out.append(("arkhdottir/" + f[:-len(".npc.snbt")], os.path.join(d0, f)))
    if tiers_out:
        emit_nbt_js(tiers_out, ROOT)

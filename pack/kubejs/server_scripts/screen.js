...
;(function () {
  ...
  // 🔴🔴 GOD 6.0 -> 14.5 on 2026-08-30, AND IT IS A BUG FIX, NOT A TUNING CHANGE.
  //
  // A god's line lasts `typing + reading` (voice.beatFor). The longest sentence in the
  // pack is 123 characters, which at the working typing rate is about 14.2 SECONDS on
  // screen. This table said 6.0, so the model believed every long line cost less than
  // half what it did - and the model is what decides whether the next thing may speak.
  //
  // ⚠️ AN UNDER-ESTIMATE HERE IS NOT SAFE. It grants the screen to something that then
  // queues behind a line still being typed, which is exactly the "delivered to a corpse"
  // failure this file exists to prevent. The cap must cover the worst real case.
  //
  // 🔑 AND ANNOUNCE MOVED WITH IT (9.0 -> 15.0), because the boot invariant is
  // `HOLD[lo] + GAP <= P[hi]`: a god holding 14.5s costs 15s of queue, and a warning that
  // tolerated less would be REFUSED behind a god mid-sentence. The audit at the bottom of
  // this file is what refuses to let that ship.
  //
  // ⚠️ BOTH NUMBERS ARE DERIVED FROM `voice.TYPE_CHARS_PER_SEC`, which is an ESTIMATE
  // from an eyeball reading. If that knob moves, these move with it - and the audit will
  // say so at boot if they are left behind.
  var HOLD = {
